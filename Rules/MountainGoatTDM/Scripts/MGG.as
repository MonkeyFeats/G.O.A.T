...
//MGG gamemode logic script

#define SERVER_ONLY

#include "MGG_Structs.as";
#include "RulesCore.as";
#include "RespawnSystem.as";

//edit the variables in the config file below to change the basics
// no scripting required!
string cost_config_file = "mgg_vars.cfg";

void Config(MGGCore@ this)
{
	CRules@ rules = getRules();

	//load cfg
	if (rules.exists("mgg_costs_config"))
		cost_config_file = rules.get_string("mgg_costs_config");
	...
	// modifies if the fall damage velocity is higher or lower - MGG has lower velocity
	rules.set_f32("fall vel modifier", cfg.read_f32("fall_dmg_nerf", 1.3f));
}

//MGG spawn system

shared class MGGSpawns : RespawnSystem
{
	MGGCore@ MGG_core;
	...
	void SetCore(RulesCore@ _core)
	{
		...
		@MGG_core = cast < MGGCore@ > (core);
	}

	void Update()
	{
		for (uint team_num = 0; team_num < MGG_core.teams.length; ++team_num)
		{
			MGGTeamInfo@ team = cast < MGGTeamInfo@ > (MGG_core.teams[team_num]);

			for (uint i = 0; i < team.spawns.length; i++)
			{
				MGGPlayerInfo@ info = cast < MGGPlayerInfo@ > (team.spawns[i]);
	...
	void UpdateSpawnTime(MGGPlayerInfo@ info, int i)
	{
		//default
		...
		bool no_respawn = MGG_core.rules.isMatchRunning() ? huge_respawn : false;
		if (no_respawn)
		{
			spawn_property = 253;
		}
		...
		string propname = "mgg spawn time " + info.username;
		MGG_core.rules.set_u8(propname, spawn_property);
		if (info !is null && info.can_spawn_time >= 0)
		{
			MGG_core.rules.SyncToPlayer(propname, getPlayerByUsername(info.username));
		}
	}
	...
	{
		MGGPlayerInfo@ info = cast < MGGPlayerInfo@ > (p_info);

		if (info is null) { warn("MGG LOGIC: Couldn't get player info ( in bool canSpawnPlayer(PlayerInfo@ p_info) ) "); return false; }
	...
	Vec2f getSpawnLocation(PlayerInfo@ p_info)
	{
		...
		if (getBlobsByName("mgg_spawn", @spawns))
		{
	...
	void RemovePlayerFromSpawn(PlayerInfo@ p_info)
	{
		...
		if (info is null) { warn("MGG LOGIC: Couldn't get player info ( in void RemovePlayerFromSpawn(PlayerInfo@ p_info) )"); return; }

		string propname = "mgg spawn time " + info.username;

		for (uint i = 0; i < MGG_core.teams.length; i++)
		{
			MGGTeamInfo@ team = cast < MGGTeamInfo@ > (MGG_core.teams[i]);
		...
		MGG_core.rules.set_u8(propname, 255);   //not respawning
		MGG_core.rules.SyncToPlayer(propname, getPlayerByUsername(info.username));
	...
	void AddPlayerToSpawn(CPlayer@ player)
	{
		...
		u32 tickspawndelay = u32(MGG_core.spawnTime);

//		print("ADD SPAWN FOR " + player.getUsername());
		MGGPlayerInfo@ info = cast < MGGPlayerInfo@ > (core.getInfoFromPlayer(player));

		if (info is null) { warn("MGG LOGIC: Couldn't get player info  ( in void AddPlayerToSpawn(CPlayer@ player) )"); return; }

		if (info.team < MGG_core.teams.length)
		{
			MGGTeamInfo@ team = cast < MGGTeamInfo@ > (MGG_core.teams[info.team]);
...
shared class MGGCore : RulesCore
{
	s32 warmUpTime;
	s32 gameDuration;
	s32 spawnTime;
	...
	MGGSpawns@ mgg_spawns;

	MGGCore() {}

	MGGCore(CRules@ _rules, RespawnSystem@ _respawns)
	{
	...
	void Setup(CRules@ _rules = null, RespawnSystem@ _respawns = null)
	{
		...
		@mgg_spawns = cast < MGGSpawns@ > (_respawns);
	...
	void Update()
	{
		...
		else if (ticksToStart > 0 && rules.isWarmup()) //is the start of the game, spawn everyone + give mats
		{
			...
			if (players_in_small_team == -1 || (getGameTime() % 30) == 4)
			{
				...
				{
					MGGTeamInfo@ team = cast < MGGTeamInfo@ > (teams[team_num]);
	...
	void updateHUD()
	{
		...
		for (uint team_num = 0; team_num < teams.length; ++team_num)
		{
			...
			{
				MGGPlayerInfo@ player = cast < MGGPlayerInfo@ > (players[player_num]);
			...
			{
				u32 st = cast < MGGPlayerInfo@ > (team.spawns[0]).can_spawn_time;
				if (st < 200)
				{
		...
		rules.set_CBitStream("mgg_serialised_team_hud", serialised_team_hud);
		rules.Sync("mgg_serialised_team_hud", true);
	}
	...
	{
		MGGTeamInfo t(teams.length, team.getName());
		teams.push_back(t);
	}

	void AddPlayer(CPlayer@ player, u8 team = 0, string default_config = "")
	{
		MGGPlayerInfo p(player.getUsername(), player.getTeamNum(), player.isBot() ? "goat" : (XORRandom(512) >= 256 ? "goat" : "goat"));
		players.push_back(p);
	...
	void SetupBases()
	{
		const string base_name = "mgg_spawn";
		...
		if (map !is null)
		{
			...
			{
				warn("MGG: Blue spawn marker not found on map");
			...
			{
				warn("MGG: Red spawn marker not found on map");
		...
		if (sudden_death)
		{
			...
			if (teams_alive_count == 1)
			{
				for (int i = 0; i < teams.length; i++)
				{
					if (teams_alive[i])
					{
						@winteam = cast < MGGTeamInfo@ > (teams[i]);
						winteamIndex = i;
						team_wins_on_end = i;
					}
				}
	...
	void addKill(int team)
	{
		...
		{
			MGGTeamInfo@ team_info = cast < MGGTeamInfo@ > (teams[team]);
			team_info.kills++;
		}
	}
